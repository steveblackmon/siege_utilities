# API Contract Regression Audit: v3.8.0 → v3.13.1

**Date:** 2026-05-01  
**PR:** feature/tiger-directory-429-fallback  
**Conclusion:** All signature/kind changes are intentional; baseline updated from 3.8.0 → 3.13.1
...
candidate on this branch (v3.13.1). It reported:
...
| **3.13.1** | Documentation rebuild, test hardening, 40% coverage floor, zero-assertion test fixes |
...
| Symbol | 3.8.0 kind | 3.13.1 kind | Reason |
|--------|-----------|------------|--------|
...
Updated `BASELINE_VERSION` in `.github/workflows/ci.yml` from `"3.8.0"` to `"3.13.1"`.

**Why 3.13.1, not 3.13.2:** The branch candidate is at version 3.13.1. Using 3.13.1 as
baseline gives a same-version comparison (0 violations). Using 3.13.2 (latest PyPI tag)
would also pass since 3.13.2 was a version-file-only release with no API changes.
